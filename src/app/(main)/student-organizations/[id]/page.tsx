...
export default function StudentOrganizationDetailsPage() {
    ...
    const { data: members, isLoading: membersLoading } = useCollection<Student>(
        useMemoFirebase(() => {
    ...
    const { data: leader, isLoading: leaderLoading } = useDoc<Student>(
        useMemoFirebase(() => org?.leaderId ? doc(firestore, 'users', org.leaderId) : null, [firestore, org?.leaderId])
    );
    ...
    if (!org) {
        return <div className="text-center py-16">Tələbə təşkilatı tapılmadı.</div>
    }
